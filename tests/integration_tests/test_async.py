...
async def test_entity_extraction_with_reask(mocker):
    ...
    assert first.inputs.messages[0]["content"] == Prompt(entity_extraction.NON_OPENAI_COMPILED_PROMPT)
    # Same as above
    assert call.compiled_messages[0]["content"]._source == entity_extraction.NON_OPENAI_COMPILED_PROMPT
    ...
    assert final.inputs.messages[1]["content"] == Prompt(
        entity_extraction.NON_OPENAI_COMPILED_PROMPT_REASK
    )
    # Same as above
    assert call.reask_messages[0][1]["content"]._source == entity_extraction.NON_OPENAI_COMPILED_PROMPT_REASK
