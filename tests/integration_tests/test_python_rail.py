...
from guardrails.validators import ValidLength, ValidChoices, register_validator, Validator, EventDetail
...
class IsValidDirector(Validator):
    def validate(self, key, value, schema) -> dict:
        valid_names = ["Christopher Nolan", "Steven Spielberg", "Martin Scorsese", "Quentin Tarantino",
                                "James Cameron"]
        if value not in valid_names:
            raise EventDetail(
                ...
                value,
                schema,
                f"Value {value} is not a valid director name. Valid choices are {valid_names}.",
                None,
            )
        return schema
...
def test_python_rail(mocker):
    ...
    expected_gd_output = json.loads(python_rail.LLM_OUTPUT_2_SUCCEED_GUARDRAILS_BUT_FAIL_PYDANTIC_VALIDATION)
    ...
    assert guard_history[0].prompt == gd.Prompt(python_rail.COMPILED_PROMPT_1_WITHOUT_INSTRUCTIONS)
    assert guard_history[0].output == python_rail.LLM_OUTPUT_1_FAIL_GUARDRAILS_VALIDATION

    assert guard_history[1].prompt == gd.Prompt(python_rail.COMPILED_PROMPT_2_WITHOUT_INSTRUCTIONS)
    assert guard_history[1].output == python_rail.LLM_OUTPUT_2_SUCCEED_GUARDRAILS_BUT_FAIL_PYDANTIC_VALIDATION

    with pytest.raises(ValueError):
        Director.parse_raw(python_rail.LLM_OUTPUT_2_SUCCEED_GUARDRAILS_BUT_FAIL_PYDANTIC_VALIDATION)

    # The user can take corrective action based on the failed validation. Either manipulating the output themselves,
    # taking corrective action in their application, or upstreaming their validations into Guardrails.
