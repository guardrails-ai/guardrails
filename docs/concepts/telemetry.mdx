...
To talk to an OTLP collector, you need only add a few environment variables and set the `tracer` attribute of your Guard to the default OTLP tracer provided by Guardrails.
...
Then, add the tracer provider to your guard:
...
guard.configure(tracer=default_otlp_tracer("my_guard"))
