...
- To see the full list of supported quality criteria, check out the [Validation](https://guardrailsai.com/guardrails/docs/concepts/validators) page.
...
| `refrain` | Refrain from returning an output after the validator returns a failure. The final validated output is replaced with the empty value for the output type, such as `None` for JSON output.                |
| `noop`    | Do not change the returned output after a validator failure. The failure will still be recorded in the logs, but no corrective action will be taken.                                                    |
