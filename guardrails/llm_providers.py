    ...
    def __call__(self, *args, **kwargs):
        try:
            ...
            raise PromptCallableException(
                f"The callable `fn` passed to `Guard(fn, ...)` failed with the following error: {e}. "
                "Make sure that `fn` can be called as a function that takes in a single prompt string "
                "and returns a string."
            )
        if not isinstance(result, str):
            raise PromptCallableException(
                f"The callable `fn` passed to `Guard(fn, ...)` returned a non-string value: {result}. "
                "Make sure that `fn` can be called as a function that takes in a single prompt string "
                "and returns a string."
            )
        return result
