...
class Guard(Runnable, Generic[OT]):
    ...
    def from_rail(
        ...
        if rail.output_type == "str":
            return cast(Guard[str], cls(rail=rail, num_reasks=num_reasks, tracer=tracer))
    ...
    def from_pydantic(
        ...
        return cast(
            Guard[Dict], cls(rail, num_reasks=num_reasks, base_model=output_class, tracer=tracer)
        )
