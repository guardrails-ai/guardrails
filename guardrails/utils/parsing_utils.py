...
def has_code_block(string_value: str, code_type: str = '') -> (bool, Optional[int], Optional[int]):
    """Checks if a string contains a code block denoted by leading and trailing tripple ticks (```)
    with an optional code type for the opening tag.
    ...
    if block_start_index != -1:
        block_end_index = string_value.find(block_border, (block_start_index + len(block_start_border)))
        return (True, block_start_index, block_end_index) if block_end_index != -1 else (False)
    return (False, None, None)

def get_code_block(string_value: str, start: int, end: int, code_type: Optional[str] = '') -> str:
    """Takes a string, start and end indexes, and an optional code type to extract a code block from a string.
