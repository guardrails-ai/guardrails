...
class Foo:
    ...
    path: Optional[TypedList["Foo"]] = field(default_factory=list)
    ...
    @classmethod
    def from_xml(cls, xml: ET.Element) -> "Foo":
        """
        Convert an XML element to a Foo object.
        ...
        try:
            foo_subtype: Foo = registry[xml.tag]
        except KeyError:
            raise KeyError(f"Element type {xml.tag} is not valid.")

        # Parse the `format` attribute of the element to build
        # the list of validators.
        format_attr = FormatAttr.from_element(xml)
        validators = format_attr.get_validators(bool(xml.attrib["strict"]))

        # Recursively parse the children of the element to build the
        # tree.
        children = [Foo.from_xml(child) for child in xml]
        ...
        return foo_subtype(
            ...
            strict=bool(xml.attrib["strict"]),
        )

    @classmethod
    def from_pydantic(self, model: BaseModel) -> "Foo":
        """
        Convert a Pydantic model to a Foo object.
        ...
        Returns:
            A Foo object.
        """
        return create_foo_for_base_model(model)
    ...
    def to_xml(self) -> ET.Element:
        ...
        element = E(self._foo_type, name=self.name)

        if self.description:
            element.attrib["description"] = self.description
        if self.validators:
            element = edit_element_to_add_validators(element, self.validators)
        if self.children:
...
def create_foo_for_field(
    ...
    try:
        sub_foo_type = registry.get(field_type)
    except KeyError:
    ...
    validators = add_validators_to_foo(field)
    ...
    # Create Foos for the field's children
    children = {}
    if field_type in ["list", "object"]:
        ...
        if is_list(type_annotation):
            ...
            if is_pydantic_base_model(inner_type):
                object_foo = create_foo_for_base_model(inner_type)
                # element.append(object_foo)
                children["item"] = object_foo
            else:
                inner_foo = create_foo_for_field(inner_type)
                # element.append(inner_foo)
                children["item"] = inner_foo

        elif is_dict(type_annotation):
            if is_pydantic_base_model(type_annotation):
                return create_foo_for_base_model(
                    type_annotation,
                    name=name,
                ...
                if len(dict_args) == 2:
                    ...
                    inner_element = create_foo_for_field(val_type)
...
def create_foo_for_base_model(
    model: BaseModel,
    name: Optional[str] = None,
    description: Optional[str] = None,
    validators: Optional[TypedList[Validator]] = [],
) -> "Foo":
    """Create a Foo object for a Pydantic BaseModel.

    This function does the following:
        1. Iterates through fields of the model and creates Foo objects for each field.
        2. If a field is a Pydantic BaseModel, it creates a Foo object that
            internally nests other Foo objects.
        3. If the BaseModel contains a field with a `when` attribute, it creates
           special `Choice` and `Case` elements for the field. # TODO

    Args:
        model: The Pydantic BaseModel to create a Foo object for
    ...
    for field_name, field in model_fields.items():
        if field_name in choice_elements or field_name in case_elements:
            continue
        field_foo = create_foo_for_field(field, field_name)
        children[field_name] = field_foo
    ...
    for when, discriminator_fields in choice_elements.items():
        ...
        for field_name, field in discriminator_fields:
            ...
            # field_foo = create_foo_for_field(field, field_name)
            # case_element.append(field_foo)
            field_foo = create_foo_for_field(field)
            case_foo = Case(name=field_name, children=[field_foo])
            choice_foo_children[field_name] = case_foo
            # choice_element.append(case_element)

        choice_foo = Choice(
            name=when, on_fail_choice="exception", children=choice_foo_children
        )
        # element.append(choice_element)
        children[when] = choice_foo
...
class ScalarType(Foo):
    pass

class NonScalarType(Foo):
    pass
...
class Date(ScalarType):
    ...
    @classmethod
    def from_xml(cls, xml: ET._Element) -> "Foo":
        foo = super().from_xml(xml)

        if "date-format" in xml.attrib:
            foo.date_format = xml.attrib["date-format"]
...
class Time(ScalarType):
    ...
    def from_xml(cls, xml: ET._Element) -> "Foo":
        ...
        if "time-format" in xml.attrib:
            foo.date_format = xml.attrib["time-format"]
